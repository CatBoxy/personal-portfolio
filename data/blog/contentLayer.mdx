...
Busqué y di con muchas opciones pero nada me convenció. Hasta que encontré
una herramienta interesante, contentlayer.

Leí la docu, vi un par de ejemplos y entendí que implementando contentlayer
...
Contentlayer es un SDK de contenido, simplifica trabajar con contenido estructurado
como data en una aplicación. Transforma el contenido en data con formato JSON y genera
definiciones de tipado de TypeScript.

Todo el proceso corre como parte de Next.js dev server, lo que permite tener la data disponible
para ser consumida una vez montada la app.

Es un preprocesador de contenido que transforma e implementa tipado de TypeScript en el contenido, generando un archivo JSON.

## Setup

Esta es la configuracion basica que arme para manejar mis archivos .mdx y consumirlos desde mi blog.
Al final de esta seccion dejo un repo con el codigo mostrado aca.
...
Instalamos Contentlayer, el plugin de Next.js, <ins>[rehype-prism-plus](https://www.npmjs.com/package/rehype-prism-plus)</ins> y <ins>[reading-time](https://www.npmjs.com/package/reading-time)</ins> que seran utiles mas adelante:

```js
npm install contentlayer next-contentlayer rehype-prism-plus reading-time
```

Envolvemos el archivo next.config.js con withContentLayer. Deberia quedar asi:
...
Si no existe, creamos ts.config.json o js.config.json en el root y agregamos las siguientes lineas. De esta manera le
estamos diciendo a nuestra aplicación donde estan nuestros archivos generados por Contentlayer.
...
Creamos el archivo contentlayer.config.js en el root. Cada Documento es una pieza individual de contenido
que Contentlayer transforma en data consumible por nuestros componentes.
Aca definimos el tipado de cada Documento, en este caso lo llamamos Post.
...
const Post = defineDocumentType(() => ({
  name: 'Post',
// Indicamos que nuestros archivos estaran en el path post/*.mdx
...
Mostramos nuestros Posts listados en index.jsx. Podemos importar todos nuestros Posts generados
por Contentlayer desde la carpeta 'contentlayer/generated'. Utilizamos getStaticProps de Next para consumir allPosts
y pasarlo por props al componente Home.
...
que definimos en contentlayer.config.js.
...
Creamos archivo [slug].js para renderizar el contenido de cada post. Next.js utiliza esta convencion para nombrar archivos dinamicos.
